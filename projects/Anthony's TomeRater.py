class User(object):
    ...
    def change_email(self, new_email):
        ...
        return "This user's emial is being changed to: {email}".format(email=new_email)
...
class Book(object):
    ...
    def set_isbn(self, new_isbn):
        self.isbn = new_isbn
        return "This book's ISBN is being changed to: {isbn}".format(isbn=new_isbn)
...
class TomeRater(object):
    ...
    def add_book_to_user(self, book, email, rating=None):
        user = self.users.get(email, "No user with the email: {email}".format(email=email))
        ...
        if number > 0:
            ...
            if number > len(self.books):
                print("Only printed: {count}\n"
                      "Did not print the remaining {remaining}, because the list is too short."
